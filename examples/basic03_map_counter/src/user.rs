...
fn load_bpf(interface: &interface::Interface, bpf_program_path: &Path, prog_sec: &str, xdp_flags: &[xdp::XdpFlags]) -> Result<rebpf::BpfObject, rebpf_error::Error> {
    let (bpf_object, _bpf_fd) = rebpf::bpf_prog_load(bpf_program_path, rebpf::BpfProgType::XDP)?;
    let bpf_prog = rebpf::bpf_object__find_program_by_title(&bpf_object, prog_sec)?.ok_or(rebpf_error::Error::InvalidProgSec)?;
    ...
    println!("Success Loading\n XDP progsec: {}, prog name: {}, id {} on device: {}", prog_sec, info.name()?, info.id(), interface.ifindex());

    Ok(bpf_object)
}

fn find_map_by_fd<T, U>(bpf_object: &rebpf::BpfObject, map_name: &str) -> Result<rebpf::BpfMapFd<T, U>, rebpf_error::Error> {
    let bpf_map = rebpf::bpf_object__find_map_by_name(bpf_object, map_name)?.ok_or(rebpf_error::Error::InvalidMapName)?;

    rebpf::bpf_map__fd(&bpf_map)
}

fn check_map_fd_info<T, U>(bpf_map_fd: &rebpf::BpfMapFd<T, U>, map_expected: &rebpf::BpfMapInfo) -> Result<rebpf::BpfMapInfo, rebpf_error::Error> {
    let map_info = rebpf::bpf_obj_get_info_by_fd(bpf_map_fd)?;
    if map_expected.type_() as u32 != map_info.type_() as u32 {
        return Err(rebpf_error::Error::CustomError("Error occured in map key size.".to_string()));
    }
    if map_expected.key_size() != map_info.key_size() {
        return Err(rebpf_error::Error::CustomError("Error occured in map key size.".to_string()));
    }
    if map_expected.max_entries() != map_info.max_entries() {
        return Err(rebpf_error::Error::CustomError("Error occured in map max entries.".to_string()));
    }
    if map_expected.value_size() != map_info.value_size() {
        return Err(rebpf_error::Error::CustomError("Error occured in map value size.".to_string()));
    }
...
fn map_collect(bpf_map_fd: &rebpf::BpfMapFd<u32, DataRec>, map_type: &rebpf::BpfMapType, key: u32, rec: &mut Record) {
...
fn stats_poll(bpf_map_fd: &rebpf::BpfMapFd<u32, DataRec>, map_type: &rebpf::BpfMapType, interval: u64) {
...
fn stats_print(stats_rec: &StatsRecord, stats_prev: &StatsRecord) {
    ...
    if period.is_some() {
        let packets = (rec.total.rx_packets.load(Relaxed) - prev.total.rx_packets.load(Relaxed)) as f64;
        let time = period.unwrap();
        let pps = packets / time.as_secs_f64();
        println!("Action: {:?}, packets: {}, pps: {}, period: {:?}", xdp::XdpAction::PASS, packets, pps, time)
    }
}

fn run(bpf_program_path: &Path, interface_name: &str, prog_sec: &str, map_name: &str, unload_program: bool) -> Result<(), rebpf_error::Error> {
    ...
    let map_expect = rebpf::BpfMapDef::<u32, DataRec>::new(rebpf::BpfMapType::ARRAY, MAX_ENTRIES).to_bpf_map_info();
    let map_info = check_map_fd_info(&stats_map_fd, &map_expect)?;
    println!("\nCollecting stats from BPF map");
    println!("- BPF map (bpf_map_type:{:?}) id:{} name:{} key_size:{}, value_size:{}, max_entries:{}", map_info.type_(), map_info.id(), map_info.name()?, map_info.key_size(), map_info.value_size(), map_info.max_entries());
...
fn main() {
    ...
    match run(&bpf_program_path, interface_name, DEFAULT_PROG_SEC, DEFAULT_MAPNAME, unload_program) {
        Err(err) => println!("{:?}", err),
        Ok(_) => {}
    };
}
